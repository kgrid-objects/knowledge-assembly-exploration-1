...
The PGx-KA can be used to compute the following two things in sequence. First, it leverages computable implementations of knowledge to map an individual's genotype information for a group of 7 (SEVEN) genes. Second, it leverages computable implementations of knowledge about pharmacogenomic phenotypes to compute drug selection or dosing recommendations for 29 (TWENTY-NINE) drugs based on computed phenotypes. For 3 (THREE) drugs, recommendations can be generated from the 29 Clinical Pharmacogenomics Implementation Consortium (CPIC) guidelines in the United States AND FROM 3 other guidelines of the Dutch Pharmacogenomics Working Group (DPWG) in the Netherlands.
